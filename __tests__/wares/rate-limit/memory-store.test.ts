...
const promisifyIncreaseStoreCb = (increaseFunction, key): Promise<{ current: number }> => {
...
describe('MemoryStore store', () => {
  it('sets the value to 1 on first incr', async () => {
    ...
    assertCurrentAfterIncrement(await promisifyIncreaseStoreCb(store.incr, key), 1)
  })

  it('increments the key for the store each increment', async () => {
    const store = new MemoryStore(-1)

    await promisifyIncreaseStoreCb(store.incr, key)
    assertCurrentAfterIncrement(await promisifyIncreaseStoreCb(store.incr, key), 2)
  })
