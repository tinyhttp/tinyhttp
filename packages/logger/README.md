...
- `outputConfiguartion.output` - function that recieved the log generated by the logger package
- `outputConfiguartion.color` - property that determines if the logger will generate a message with color. useful for logging into the console, disable it if logging into file / http / etc...
...
app
  .use(logger({ methods: ['GET', 'POST'], timestamp: { format: 'HH:mm:ss' }, outputConfiguration: { output: console.log, color: false } }))
