...
it('should chain middleware', () => {
  ...
  expect(app.middleware.length).toEqual(2)
})
...
it('app.locals are get and set', () => {
  ...
  expect(app.locals.hello).toEqual('world')
})
...
it('`app.set` sets a setting', () => {
  ...
  expect(app.settings.subdomainOffset).toEqual(1)
})
...
it(`app.enable enables a setting`, () => {
  ...
  expect(app.settings.xPoweredBy).toEqual(true)
})
it(`app.disable disables a setting`, async () => {
  ...
  expect(app.settings.xPoweredBy).toEqual(false)
})
...
it('sub-app mounts on a specific path', () => {
  ...
  expect(subApp.mountpath).toEqual('/subapp')
})
...
it('should mount on "/" if path is not specified', () => {
  ...
  expect(subapp.mountpath).toEqual('/')
})
/* it('app.parent should reference to the app it was mounted on', () => {
  ...
  expect(subapp.parent).toEqual(app)
}) */
it('app.path() should return the mountpath', () => {
  ...
  expect(subapp.path()).toEqual('/subapp')
})
it('app.path() should nest mountpaths', () => {
  ...
  expect(subsubapp.path()).toEqual('/blog/admin')
})
it('middlewares of a subapp should preserve the path', () => {
  ...
  expect(subapp.middleware[0].path).toEqual('/path')
})
...
it = suite('App settings')

it('xPoweredBy: is enabled by default', () => {
  const app = new App()

  expect(app.settings.xPoweredBy).toEqual(true)
})
it('xPoweredBy: should set X-Powered-By to "tinyhttp"', async () => {
  const { fetch } = InitAppAndTest((_req, res) => void res.send('hi'))

  await fetch('/').expectHeader('X-Powered-By', 'tinyhttp')
})
it('xPoweredBy: when disabled should not send anything', async () => {
...
it('bindAppToReqRes: references the current app instance in req.app and res.app', async () => {
  const app = new App({
    settings: {
  ...
  app.use((req, res) => {
    expect(req.app instanceof App).toEqual(true)
    expect(res.app instanceof App).toEqual(true)
    expect(req.app.locals['hello']).toEqual('world')
    expect(res.app.locals['hello']).toEqual('world')
    res.end()
  })
